...
commands
  silent
  printf "\n=== async stack snapshot at download_async.rs:25 ===\n"
  printf "a Tokio child future is being polled\n"
  info threads
  python
import gdb, os, re
...
for num, tid, count, size_sum, detail in rows:
    ...
    if detail:
        print(f"  {detail}")
print(f"SUMMARY model=async gdb_threads={len(threads)} stack_maps={total_stack_maps} mapped_stack_bytes={total_stack_bytes} mapped_stack_mib={total_stack_bytes/1024/1024:.3f} largest_stack_kib={largest_stack/1024:.1f}")
  end
  detach
  quit
end
