...
\title[AI 辅助异步状态机跟踪]{一种基于 AI 辅助的 Tokio 异步状态机动态跟踪分析}
...
\begin{frame}{整体技术路径}
  ...
  \begin{tikzpicture}[node distance=1.1cm, every node/.style={font=\large}]
    ...
    \node[draw=UCBFoundersRock, thick, rounded corners, fill=UCBGray, minimum width=3.2cm, minimum height=1.0cm, right=of raw] (ai) {2. AI 辅助分析};
  ...
  \begin{alertblock}{核心优点}
    暴露 AI 得出结果的路径：每个结论都可以回溯到命令、断点、日志、寄存器、内存地址和调用栈。
  \end{alertblock}
\end{frame}
...
\begin{frame}{第二步：AI 辅助分析}
  ...
  \begin{alertblock}{使用原则}
    AI 不直接给最终裁决，而是帮助整理证据链、发现重复模式、标注不确定性。
  \end{alertblock}
\end{frame}
...
\begin{frame}{第三步：人工提炼与校验}
  ...
  \begin{alertblock}{底层分析里的危险}
    AI 容易把所有值都解释完整；但系统调试需要保留“不知道”，否则会把临时值包装成错误结论。
  \end{alertblock}
\end{frame}
...
\begin{frame}{可解释性来自哪里}
  \begin{enumerate}
    ...
    \item AI 路径可暴露：结构化、假设、证据对照、人工校验分层清楚
...
\begin{frame}{如何更好地使用 AI}
  ...
  \begin{alertblock}{一句话}
    AI 的价值不是替代推理，而是把推理过程显性化、结构化、可复查。
  \end{alertblock}
\end{frame}
...
\begin{frame}{总结}
  ...
  \begin{block}{方法收获}
    建立了从原始日志到 AI 辅助分析，再到人工提炼报告的可解释工作流。
  \end{block}
